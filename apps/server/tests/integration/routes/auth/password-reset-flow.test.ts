...
const fakeDb = {
  async query<T>(sql: string, params: unknown[] = []): Promise<T[]> {
    ...
    if (/FROM password_resets/i.test(sql)) {
      const [tokenHash, notAfter] = params as [string, string];
      return resets
  ...
  async run(sql: string, params: unknown[] = []): Promise<void> {
    ...
    if (/UPDATE password_resets SET used_at/i.test(sql)) {
      const [usedAt, id] = params as [string, string];
    ...
    if (/UPDATE users SET password_hash/i.test(sql)) {
      const [hash, , id, siteId] = params as [string, string, string, string];
      const row = users.find((u) => u.id === id && u.site_id === siteId);
...
vi.mock("../../../../src/lib/database/db.js", () => ({ getDb: async () => fakeDb, resetDb: () => {} }));
...
const { default: authRoutes } = await import("../../../../src/routes/auth/auth.js");
...
const { hashResetToken } = await import("../../../../src/lib/auth/password-reset-db.js");
const { resetRateLimits } = await import("../../../../src/lib/security/rate-limit.js");
