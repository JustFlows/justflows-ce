...
const settings = {
  ...
  threadMaxDepth: 6,
  pageSize: 50,
  captchaProvider: "none" as "none" | "turnstile" | "recaptcha" | "recaptcha-v3",
  captchaSiteKey: "",
  captchaScoreThreshold: 0.5,
  captchaSecretKey: "",
};

vi.mock("../../../src/lib/comments/comments-settings.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../../src/lib/comments/comments-settings.js")>();
...
vi.mock("../../../src/lib/email/mail.js", () => ({ sendMail }));

const hooks = {
  has: vi.fn(() => false),
  applyFilter: vi.fn(async (_hook: string, value: unknown, _context?: unknown) => value),
};
...
beforeEach(() => {
  ...
  hooks.applyFilter.mockReset().mockImplementation(async (_hook: string, value: unknown, _context?: unknown) => value);
  resetRateLimits();
...
describe("renderCommentsBlockHtml", () => {
  ...
  it("lets a comments.render filter replace the markup and hands it the thread data", async () => {
    ...
    hooks.applyFilter.mockImplementation(async (_hook: string, _html: unknown, context: any) => {
