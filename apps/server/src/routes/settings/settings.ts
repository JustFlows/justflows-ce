...
import { THEME_CUSTOMIZE_ROLES } from "../../lib/auth/rbac.js";
import { setHomePageId } from "../../lib/content/home-page.js";
import { setBlogPageId } from "../../lib/content/blog-page.js";
import {
  sendTestMail,
  listEmailDeliveries,
  retryEmailDelivery,
...
import { getPermalinkState, savePermalinks, PermalinkConflictError } from "../../lib/navigation/permalinks-db.js";
...
router.get("/comments", requireRole("administrator"), async (_req, res) => {
  try {
    const { getCommentSettings, toPublicCommentSettings } =
      await import("../../lib/comments/comments-settings.js");
    const siteId = await getSiteId();
    if (!siteId) {
      res.status(503).json({ error: "No site found" });
      return;
...
router.put("/pwa", requireRole("administrator"), async (req, res) => {
  try {
    ...
    const { getPwaSettings, savePwaSettings, isSafePublicPath } = await import(
      "../../lib/pwa/pwa-settings.js"
    );
    const { generatePwaIconPair } = await import("../../lib/pwa/pwa-icons.js");
    ...
    const { PwaValidationError } = await import("../../lib/pwa/pwa-settings.js");
    const { PwaIconError } = await import("../../lib/pwa/pwa-icons.js");
