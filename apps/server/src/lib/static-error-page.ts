...
import { viewsDir } from "./jf-root.js";
...
const catalogDir = path.join(path.dirname(fileURLToPath(import.meta.url)), "i18n", "site-catalogs");
...
function loadStaticCatalog(locale: string): Record<string, string> {
  ...
  try {
    data = JSON.parse(fs.readFileSync(path.join(catalogDir, `${locale}.json`), "utf-8")) as Record<
      string,
      string
    >;
  } catch {
