...
function runCopiedMigrations(root: string): { ok: boolean; output: string } {
  const entry = path.join(root, "apps/server/dist/lib/database/apply-pending-migrations-cli.js");
...
function installedWorkerPath(root: string): string {
  return path.join(root, "apps/server/dist/lib/updates/core-update-worker.js");
}
  ...
  try {
    ...
    if (!workerPath && zipPath) {
      try {
        ...
        const shipped = path.join(srcRoot, "apps/server/dist/lib/updates/core-update-worker.js");
