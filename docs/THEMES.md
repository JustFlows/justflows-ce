...
`system` clears the stored value. `aria-pressed` marks the *preference* when a
...
Headings stay fluid: the Customizer's size is the *ceiling* of a `clamp()`, not
...
and credit line stay. The page header remains per-page in the page builder.
